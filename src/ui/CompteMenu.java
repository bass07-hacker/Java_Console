...
/**
 * Interface utilisateur pour la gestion des comptes et opérations financières avec design premium.
 */
public class CompteMenu {
    ...
    /**
     * Formate un montant en FCFA (ex: 50 000 FCFA).
     */
    ...
    public void showHistory() {
        ...
        try {
            ...
            if (operations.isEmpty()) {
                ...
                System.out.println(ConsoleUI.BOLD_GREEN + String.format("%-20s | %-20s | %-15s", "DATE", "TYPE", "MONTANT") + ConsoleUI.RESET);
                System.out.println(ConsoleUI.WHITE + "-".repeat(60) + ConsoleUI.RESET);
                for (Operation op : operations) {
                    System.out.printf("%-20s | %-20s | %-15s\n", 
                        dateFormat.format(op.getDate()), op.getType(), formatCurrency(op.getMontant()));
                }
            }
        } catch (CompteIntrouvableException e) {
            ConsoleUI.printError(e.getMessage());
